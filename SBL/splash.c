...
static SBL_CODE void splash_backlight_off(void) {
  splash_gpio_reset(GPIOD, SPLASH_LCD_BL_PIN);
}
...
static SBL_CODE uint8_t splash_state_valid(const SplashSblStateRecord *r) {
  if (r->magic != SPLASH_SBL_STATE_MAGIC ||
      r->version != SPLASH_SBL_STATE_VERSION) {
    return 0U;
  }
  return (r->crc == splash_state_crc(r)) ? 1U : 0U;
}

static SBL_CODE uint8_t splash_state_erased(const SplashSblStateRecord *r) {
  const uint32_t *w = (const uint32_t *)r;
  for (uint32_t i = 0U; i < sizeof(SplashSblStateRecord) / sizeof(uint32_t); ++i) {
    if (w[i] != 0xFFFFFFFFUL) {
      return 0U;
    }
  }
...
static SBL_CODE uint8_t splash_bootloader_unlocked(void) {
  const SplashSblStateRecord *latest = 0;
  for (uint32_t off = 0U;
       off + sizeof(SplashSblStateRecord) <= SPLASH_SBL_STATE_AREA_SIZE;
       off += sizeof(SplashSblStateRecord)) {
    const SplashSblStateRecord *r =
        (const SplashSblStateRecord *)((uint32_t)__sbl_state_start__ + off);
    if (splash_state_erased(r)) {
      break;
    }
    if (splash_state_valid(r)) {
      latest = r;
    }
  }
  if (!latest) {
    const SplashSblStateRecord *old =
        (const SplashSblStateRecord *)SPLASH_SBL_STATE_OLD_ADDR;
    latest = splash_state_valid(old) ? old : 0;
  }
...
static SBL_CODE void splash_lcd_init(void) {
  ...
  splash_lcd_cmd(0x01U);
  SBL_DelayMs(150U);
  splash_lcd_cmd(0x11U);
  SBL_DelayMs(120U);
  splash_lcd_cmd(0x3AU);
  splash_lcd_data(0x55U);
  splash_lcd_cmd(0x36U);
  splash_lcd_data(0x10U);
  splash_lcd_cmd(0xB2U);
  {
    uint8_t d[] = {0x0C, 0x0C, 0x00, 0x33, 0x33};
    splash_lcd_data_bytes(d, sizeof(d));
  }
  splash_lcd_cmd(0xB7U);
  splash_lcd_data(0x35U);
  splash_lcd_cmd(0xBBU);
  splash_lcd_data(0x19U);
  splash_lcd_cmd(0xC0U);
  splash_lcd_data(0x2CU);
  splash_lcd_cmd(0xC2U);
  splash_lcd_data(0x01U);
  splash_lcd_cmd(0xC3U);
  splash_lcd_data(0x12U);
  splash_lcd_cmd(0xC4U);
  splash_lcd_data(0x20U);
  splash_lcd_cmd(0xC6U);
  splash_lcd_data(0x0FU);
...
static SBL_CODE void splash_draw_logo(void) {
  ...
  if (splash_bootloader_unlocked()) {
    splash_draw_unlock_icon();
  }
}
