...
static const char sbl_damage_bad_header[] SBL_CONST =
    "System image invalid.";
static const char sbl_damage_bad_crc[] SBL_CONST =
    "System image corrupted.";
static const char sbl_damage_digest[] SBL_CONST =
    "System image mismatch.";
static const char sbl_damage_signature[] SBL_CONST =
    "System signature invalid.";
static const char sbl_damage_rollback[] SBL_CONST =
    "System version too old.";
static const char sbl_damage_default[] SBL_CONST =
...
SBL_CODE void SBL_UiRunFastboot(void) {
  ...
  uint32_t local_last_action = HAL_GetTick();
  uint32_t usb_grace_start = local_last_action;
  uint32_t usb_retry_at = local_last_action + 2500U;
...
SBL_CODE void SBL_UiRunSystemDamage(SecureBoot_Result_t reason) {
  const char *body;
  switch (reason) {
  default:                     body = sbl_damage_default;    break;
  case SECUREBOOT_VERIFY_BAD_HEADER:  body = sbl_damage_bad_header; break;
  case SECUREBOOT_VERIFY_BAD_CRC:     body = sbl_damage_bad_crc;    break;
  case SECUREBOOT_VERIFY_BAD_DIGEST:  body = sbl_damage_digest;     break;
  case SECUREBOOT_VERIFY_BAD_SIGNATURE: body = sbl_damage_signature; break;
  case SECUREBOOT_VERIFY_ROLLBACK:    body = sbl_damage_rollback;   break;
  }
  SBL_LcdDisplayOff();
  SBL_LcdRect(0U, 0U, SBL_LCD_W, SBL_LCD_H, SBL_BLACK);
  sbl_draw_text_center_line(102U, sbl_damage_title, SBL_RED, SBL_FONT_SMALL);
  sbl_draw_text_center_block(126U, body, SBL_WHITE, SBL_FONT_SMALL);
  SBL_LcdDisplayOn();
  while (1) {
    SBL_USB_Tick();
    SBL_DelayMs(10U);
...
SBL_CODE void SBL_UiRunFastboot(void) {
  uint32_t usb_grace_start = HAL_GetTick();
  uint32_t usb_retry_at = usb_grace_start + 2500U;
  uint8_t usb_grace_done = 0U;
