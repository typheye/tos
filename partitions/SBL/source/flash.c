...
static SBL_CODE uint8_t signed_partition_valid(
  ...
  if (part->address == TOS_PART_SBL_ADDRESS) {
    type = TOS_IMAGE_TYPE_SBL;
    signed_size = TOS_PART_SBL_SIZE;
  } else if (part->address == TOS_PART_REC_ADDRESS) {
    type = TOS_IMAGE_TYPE_REC;
    signed_size = TOS_PART_REC_SIGNED_SIZE;
  } else if (part->address == TOS_PART_SYSTEM_ADDRESS) {
    type = TOS_IMAGE_TYPE_SYSTEM;
    signed_size = TOS_PART_SYSTEM_SIZE;
  } else {
    return 1U;
  }
  return SecureBoot_Verify(storage_address, part->address, signed_size,
                             type, 1U) == SECUREBOOT_VERIFY_OK
             ? 1U
             : 0U;
}
...
SBL_CODE uint8_t SBL_FlashFinalize(SBL_FlashSession *session) {
  ...
  if (crc != session->expected_crc ||
      !signed_partition_valid(session->part, session->write_address)) {
    SBL_FlashAbort(session);
    return 0U;
  }
