...
#include "demo/include/hid_activity.hpp"
...
#define TOS_ITEMS 3
static const char *tos_m[TOS_ITEMS] = {"00 Return", "01 Settings", "03 HID Test"};
...
static void draw_menu(const char *title, const char **items, int count, int sel) {
  ...
  if (HAL_GetTick() - lt > 1000) {
    lt = HAL_GetTick(); Time_t t; Date_t d;
  ...
  int st = sel - v / 2; if (st < 0) st = 0; if (st + v > count) st = count - v;
  for (int i = 0; i < v; i++) {
    int idx = st + i; if (idx >= count) break;
    int cy = 33 + i * 25;
    if (idx == sel) { PD_DrawAngledCard(14, cy, 212, 20, 5, TOS_ACCENT); PD_SetColor(TOS_TEXT); }
...
static int menu_loop(const char *title, const char **items, int count, int start_sel) {
  int sel = start_sel; if (sel >= count) sel = 0;
...
void SysUI::loop(void) {
  ...
  if (now_activity == UI_LAUNCHER) {
    ...
    if (sel == 0) { now_activity = UI_PET; }
    else if (sel == 1) { settings_run(); }
    else if (sel == 2) { hid_test_activity_run(); }
