...
static void fm_unmount(void) {
  if (fm_mounted) { f_mount(NULL, "0:", 0); fm_mounted = false; }
}

static void fm_load_dir(void) {
  fm_count = 0;
  if (!fm_mounted) fm_mount();
  if (!fm_mounted) return;

  /* Always show .. for navigation/exit */
  strcpy(fm_items[0], ".."); fm_is_dir[0] = 1; fm_count = 1;

  DIR dir; FILINFO fno;
  if (f_opendir(&dir, fm_cur_path) != FR_OK) return;
  while (f_readdir(&dir, &fno) == FR_OK && fno.fname[0]) {
    if (fm_count >= FM_MAX_ITEMS - 1) break;
    int idx = fm_count++;
  ...
  /* Sort: dirs first */
  for (int i = (strcmp(fm_cur_path, "0:") == 0 ? 0 : 1); i < fm_count - 1; i++) {
    for (int j = i + 1; j < fm_count; j++) {
      bool swap = false;
      if (fm_is_dir[i] && !fm_is_dir[j]) continue;
      if (!fm_is_dir[i] && fm_is_dir[j]) swap = true;
      else if (strcasecmp(fm_items[i], fm_items[j]) > 0) swap = true;
      if (swap) {
        char tn[FM_NAME_LEN]; int td;
        strcpy(tn, fm_items[i]); td = fm_is_dir[i];
        strcpy(fm_items[i], fm_items[j]); fm_is_dir[i] = fm_is_dir[j];
        strcpy(fm_items[j], tn); fm_is_dir[j] = td;
      }
    }
  }
}
...
static void fm_enter(int idx) {
  ...
  if (strcmp(fm_items[idx], "..") == 0) {
    ...
    snprintf(fm_cur_path + len, sizeof(fm_cur_path) - len, "/%s", fm_items[idx]);
    fm_load_dir();
  }
}
...
void file_manager_run(void) {
  ...
  while (1) {
    ...
    if (keyManager.btn_enter.getState() == KEY_PRESSED) {
      if (n == 0) { fm_unmount(); return; }
      fm_enter(sel);
      if (!fm_mounted) return; /* exit via .. at root */
      n = fm_count; sel = 0;
    }
