...
static void draw_card_r(int idx, int sel, int cy, const char *label, const char *value, bool editing) {
  ...
  if (editing && selected && (HAL_GetTick() / 300) % 2) card_c = TOS_CARD_BG;
  PD_DrawAngledCard(14, cy, 212, 20, 5, card_c);
  PD_SetColor(txt_c); PD_DrawString(26, cy + 2, label);
...
static void draw_hs_main(int sel) {
  ...
  for (int i = 0; i < visible; i++) {
    ...
    int cy = 33 + i * 25;
    switch (idx) {
    case 0: draw_card(idx, sel, cy, "00 Return", false); break;
...
static int hs_main_loop(void) {
  ...
  if (sel >= hs_item_count()) sel = hs_item_count() - 1;
  hs_edit = false;
  ...
  while (1) {
    ...
    le = ce;
    if (HAL_GetTick() - lu > 100) { lu = HAL_GetTick(); draw_hs_main(sel); }
    HAL_Delay(20);
  }
}
...
static void draw_ssidpwd(int sel) {
  ...
  char buf[48];
  snprintf(buf, sizeof(buf), "SSID:%s", hs_ssid); PD_DrawString(26, 115, buf);
  snprintf(buf, sizeof(buf), "PWD: %s", hs_pwd);  PD_DrawString(26, 135, buf);
...
static void ssidpwd_run(void) {
  ...
  while (1) {
    keyManager.collision_A8.tick(); keyManager.collision_D0.tick(); keyManager.btn_enter.tick();
    if (keyManager.collision_A8.getState() == KEY_PRESSED) { sel = (sel + 1) % 3; HAL_Delay(150); }
    if (keyManager.collision_D0.getState() == KEY_PRESSED) { sel = (sel - 1 + 3) % 3; HAL_Delay(150); }
